...
int drop_priv_temp(uid_t newuid)
{
	if(setresuid(-1,newuid,geteuid())<0)
		return ERROR_SYSTEMCALL;
	if(geteuid()!=newuid)
		return ERROR_SYSTEMCALL;
	return 0;
}

int drop_priv_perm(uid_t newuid)
{
...
char buf[BUF_LEN];
read(sockfd, buf, BUF_LEN);//在这里处理相应报文的时候需要临时放弃权限
if(0!=drop_priv_temp(1000))
{
	printf("drop_priv_temp_error!!!\n");
}
...
void main(){
printf("son_p afterexec:::uid:%d,euid:%d",getuid(),geteuid());
if(0!=drop_priv_temp(1000))
{
	printf("drop_priv_temp_error!!!\n");
}
...
//端口号一定要使用htons先转化为网络字节序，否则绑定的实际端口
...
if(0!=drop_priv_perm(1000))
{
	printf("drop_priv_perm_error!!!\n");
}
//监听功能关闭，永久放弃权限
}
